...
def run_daily_pipeline(hours: int = 24, top_n: int = 10) -> dict:
    ...
    try:
        ...
        logger.info(
            f"✓ Processed {anthropic_result['processed']} "
        ...
        logger.info(
            f"✓ Processed {youtube_result['processed']} "
            f"transcripts "
        ...
        digest_result = process_digests(limit=20)

        results["digests"] = digest_result

        logger.info(
            f"✓ Created {digest_result['processed']} digests "
            f"({digest_result['failed']} failed "
            f"out of {digest_result['total']} total)"
        )
    ...
    logger.info(
        f"Scraped: {results['scraping']}"
    )

    logger.info(
        f"Processed: {results['processing']}"
    )

    logger.info(
        f"Digests: {results['digests']}"
    )

    logger.info(
        f"Email: {'Sent' if results['success'] else 'Failed'}"
    )
